...
export class SyncEngine {
  ...
  async syncDevice(
    ...
    try {
      ...
      // For "latest" mode, determine the cutoff date from the last synced record.
      // We go 1 day before the last record to ensure no gaps, then only process
      // records from that point onwards — avoiding re-inserting thousands of old records.
      let latestCutoffDate: string | null = null;
      if (options.mode === 'latest') {
        const lastTimestamp = await getLatestLogTimestamp(deviceId);
        if (lastTimestamp) {
          const lastDate = new Date(lastTimestamp);
          lastDate.setDate(lastDate.getDate() - 1); // Go 1 day before
          latestCutoffDate = lastDate.toISOString().split('T')[0] as string;
          console.log(`[SyncEngine] Latest mode: last record at ${lastTimestamp}, cutoff date: ${latestCutoffDate}`);
        } else {
          console.log('[SyncEngine] Latest mode: no existing records, will process all fetched logs');
        }
      }
      ...
      try {
        ...
        // Apply "latest" mode filter: only keep records from cutoff date onwards
        if (latestCutoffDate && deviceLogs.length > 0) {
          deviceLogs = deviceLogs.filter(log => {
            const logDate = log.timestamp.split('T')[0] ?? '';
            return logDate >= latestCutoffDate!;
          });
          console.log(`[SyncEngine] Latest mode: filtered ${totalFetched} → ${deviceLogs.length} logs (from ${latestCutoffDate})`);
        }
        ...
        console.log(`[SyncEngine] Received ${deviceUsers.length} users and ${totalFetched} logs from device (processing ${deviceLogs.length})`);
